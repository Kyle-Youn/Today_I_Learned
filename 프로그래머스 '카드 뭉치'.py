...
# 'goal' 리스트 요소의 개수 n에 대한 시간복잡도 O(n)
def solution(cards1, cards2, goal):
    pointer1 = 0    # cards1 리스트에 대한 포인터
    pointer2 = 0    # cards2 리스트에 대한 포인터
    len1 = len(cards1)
    len2 = len(cards2)
    for word in goal:    # goal 리스트 요소 순회
        if pointer1 < len1 and cards1[pointer1] == word:
            pointer1 += 1
            continue
        elif pointer2 < len2 and cards2[pointer2] == word:
            pointer2 += 1
            continue
        else:
            return "No"    # goal 요소를 순회할 수 없다면 "No" 리턴
    return "Yes"    # goal 요소를 전부 순회하면 "Yes" 리턴