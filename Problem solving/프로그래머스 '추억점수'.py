...
# 시간복잡도 O(n) + O(p * q) (p, q는 각각 photo 요소의 개수, )
def solution(name, yearning, photo):
    answer = []
    name_dict = {}
    
    for person, sco in zip(name, yearning):    # name과 yearning을 Key:Value로 변환
        name_dict[person] = sco
        
    for pic in photo:    # 2차원 배열의 리스트 요소를 순회하며 각 사진 별 점수를 answer 리스트에 추가
        photo_sco = 0
        for person2 in pic:
            if person2 in name_dict:
                photo_sco += name_dict[person2]
        answer.append(photo_sco)
