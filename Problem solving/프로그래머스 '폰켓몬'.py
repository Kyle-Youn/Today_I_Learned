...
# 시간복잡도 O(1)
def solution(nums):
    selec_nums = len(nums) // 2    # 선택할 수 있는 폰켓몬의 수
    set_nums = len(set(nums))    # 중복없는 포켓몬의 수
    return set_nums if set_nums <= selec_nums else selec_nums # 중복없는 포켓몬의 수와 선택할 수 있는 포켓몬의 수 중 더 작은 수를 반환