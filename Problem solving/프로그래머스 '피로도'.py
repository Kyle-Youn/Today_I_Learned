...
# 시간복잡도 O(n * n!)
import itertools    # 순열을 사용하기 위한 라이브러리 호출

def solution(k, dungeons):
    answer = 0
    
    for permute in itertools.permutations(dungeons, len(dungeons)):    # 각 던전을 순서대로 나열한 모든 경우의 수 반환
        dun_num = 0   # 경우의 수 별로 가능한 던전 개수
        cur_k = k    # 경우의 수 별 피로도
        
        for dungeon in permute:    # 순열 경우의 수의 던전들 순회
            if cur_k >= dungeon[0]:    # 남은 피로도가 최소피로도보다 클 경우
                cur_k -= dungeon[1]
                dun_num += 1
            else: break
        
        if dun_num > answer:    # 순열 경우의 수 중에서 순회하는 던전 수의 최대값 반환
            answer = dun_num
