...
# 수정본(가독성)
def solution(N, a, b):
    round_number = 1  # 현재 라운드 번호를 추적하기 위한 변수를 1로 초기화

    while True:
        # 만남의 조건 검사: A와 B가 만나려면 하나는 홀수, 다른 하나는 짝수이며 둘 사이의 차이가 1이어야 함
        if (a % 2 == 1 and b == a + 1) or (b % 2 == 1 and a == b + 1):
            return round_number
        
        # 두 참가자의 번호를 업데이트하면서 다음 라운드로 진행
        a = (a // 2) + (a % 2)
        b = (b // 2) + (b % 2)
        round_number += 1  # 라운드 번호를 증가