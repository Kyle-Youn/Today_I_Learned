...
# 1. str1의 각 부분 문자열을 사전에 저장하고 빈도수를 카운트
    for i in range(len(str1)-1):
        substr = str1[i:i+2].lower()
        if substr.isalpha():
            if substr in dict1:
                dict1[substr] += 1
            else:
...
# 2. str2의 각 부분 문자열을 사전에 저장하고 빈도수를 카운트
    for i in range(len(str2)-1):
        substr = str2[i:i+2].lower()
        if substr.isalpha():
            if substr in dict2:
                dict2[substr] += 1
            else:
...
# 3. 교집합과 합집합의 크기 계산
    intersection = 0
    union = 0
    all_keys = set(dict1.keys()).union(set(dict2.keys()))   # 모든 키를 합친 집합 생성
