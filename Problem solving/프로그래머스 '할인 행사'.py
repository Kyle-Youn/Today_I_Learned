...
def solution(want, number, discount):
    ...
    for wanna, num in zip(want, number):    # 원하는 품목과 수량을 담은 딕셔너리 생성
        want_dict[wanna] = num
    
    for start in range(len(discount) - 9):    # discount 리스트를 10개씩 슬라이싱할 때 시작지점
        tmp_dict = {}
        
        for item in discount[start:start+10]:    # 시작지점을 기준으로 discount 리스트를 10개씩 슬라이싱
            if item in tmp_dict:
                tmp_dict[item] += 1
            else:
                tmp_dict[item] = 1
        
        if  want_dict == tmp_dict:    # 두 딕셔너리가 일치할 경우 가능한 회원등록일에  1일 추가
            answer += 1
