...
def solution(k, score):
    ...
    HoF = []    # 명예의 전당
    for sco in score:
        if len(HoF) < k:    # 명예의 전당 리스트가 k보다 작을 때
            HoF.append(sco)
            answer.append(min(HoF))
        else:    # 명예의 전당 리스트가 k보다 클 때
            HoF.sort(reverse=True)    # 명예의 전당에서 가장 낮은 점수가 가장 뒤에 오도록 내림차순 정렬
            if min(HoF) < sco:
                HoF.pop()
                HoF.append(sco)
            answer.append(min(HoF))   # 갱신된 명예의 전당에서 최소값을 answer에 추가
    return answer