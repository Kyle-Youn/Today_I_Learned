...
이 프로그램의 테스팅 업무를 담당하고 있는 어피치는  서비스를 오픈하기전 각 로직에 대한 성능 측정을 수행하였는데, 
...
def solution(cacheSize, cities):
    run_time = 0
    cache_deq = deque()    # 연결리스트 사용. 양 쪽 끝 추가, 제거 연산 시간복잡도 O(1)
    ...
    for city in cities
        ...
        if city in cache_deq:    # city가 캐시에 있을 때
            run_time += 1
            cache_deq.remove(city)
            cache_deq.append(city)          
        else:    # 캐시가 city에 없을 때
            if len(cache_deq) >= cacheSize:
                cache_deq.popleft()
                cache_deq.append(city)
                run_time += 5
            else:
