...
def solution(operations):
    ...
    for oper in operations:    # operations 순회
        if oper[0] == 'I':    # 삽입 연산인 경우
            heapq.heappush(min_heap, int(oper[2:]))
            heapq.heappush(max_heap, -int(oper[2:]))
        elif oper == "D -1" and min_heap:    # 최소값 삭제 연산인 경우
            min_val = heapq.heappop(min_heap)
            max_heap.remove(-min_val)
        elif oper == "D 1" and min_heap:    # 최대값 삭제연산인 경우
