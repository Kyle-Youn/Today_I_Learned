...
def solution(data, ext, val_ext, sort_by):
    answer = []
    row_info = ['code', 'date',	'maximum', 'remain']
    ext_idx = row_info.index(ext)
    sort_by_idx = row_info.index(sort_by)
    
    for row in data:    # data의 각 요소를 순회하며 조건에 맞는 리스트만 추출하여 answer리스트에 append
        if row[ext_idx] < val_ext:
            answer.append(row)
            
    answer.sort(key=lambda x:x[sort_by_idx])    # 조건에 맞도록 정렬    
